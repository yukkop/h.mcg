use crate::graph::{
    plugin::node_system,
    structs::{Dot, DotBundle, LineBundle, NodeBundle, OldNode},
};
...
impl Plugin for MapPlugin {
    fn build(&self, app: &mut App) {
        app.add_system(node_system)
            .add_startup_system(setup_map_system);
    }
}
            ...
            for (index_i, node) in nodes.iter().enumerate() {
                ...
                parent
                    .spawn(MaterialMesh2dBundle {
                        ...
                        material: materials.add(ColorMaterial::from(ColorMaterial::from(
...
pub fn setup_map_old(
    ...
    nodes.push(OldNode {
        dot: Dot(Vec2::new(100., 100.)),
        connections: Vec::new(),
    });
    nodes.push(OldNode {
        dot: Dot(Vec2::new(-100., 200.)),
        connections: Vec::new(),
    });
    nodes.push(OldNode {
        dot: Dot(Vec2::new(300., 200.)),
        connections: Vec::new(),
    });
    nodes.push(OldNode {
        dot: Dot(Vec2::new(200., -150.)),
        connections: Vec::new(),
    });
    nodes.push(OldNode {
        dot: Dot(Vec2::new(150., -100.)),
        connections: Vec::new(),
    });
    let dot = nodes[1].dot;
            ...
            for node in nodes {
                ...
                parent
                    .spawn(MaterialMesh2dBundle {
                        mesh: meshes.add(Mesh::from(shape::Quad::default())).into(),
                        transform: Transform::from_translation(Vec3::new(
                            node.dot.x, node.dot.y, 2 as f32,
                        ))
                        .with_scale(Vec3::splat(12.)),
                        material: materials.add(ColorMaterial::from(ColorMaterial::from(
                            Color::hex("ffffff").unwrap(),
